"""Print a compact markdown table for the WikiText-103 comparison logs."""
...
def main():
    logs_dir = Path("logs")
    configs = {
        "Baseline": "wikitext_baseline.jsonl",
        "Per-Token (Many-Small)": "wikitext_per_token.jsonl",
        "Recurrent (Few-Rich)": "wikitext_recurrent.jsonl",
    }
    ...
    if not rows:
        print("No WikiText log files found in logs/. Run experiments/run_wikitext_comparison.py first.")
        return
